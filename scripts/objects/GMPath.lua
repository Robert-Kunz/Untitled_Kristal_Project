...
function GMPath:getTotalLength()
    local total_length = 0
    for i = 1, #self.intpoints - 1 do
        local point_a = self.intpoints[i]
        local point_b = self.intpoints[i + 1]
        total_length = total_length + Utils.dist(point_a.x, point_a.y, point_b.x, point_b.y)
    end
    return total_length
end

function GMPath:getPositionAtLength(length)
    local totalLength = self:getTotalLength()

    if length <= 0 then
        return self.intpoints[1]
    elseif length >= totalLength then
        return self.intpoints[#self.intpoints]
    end
    ...
    for i = 1, #self.intpoints - 1 do
        local point_a = self.intpoints[i]
        local point_b = self.intpoints[i + 1]
        local segment_length = Utils.dist(point_a.x, point_a.y, point_b.x, point_b.y)

        if accumulated_length + segment_length >= length then
            local remainingLength = length - accumulated_length
            local t = remainingLength / segment_length
            return self:createPoint(
                point_a.x + t * (point_b.x - point_a.x),
                point_a.y + t * (point_b.y - point_a.y),
                point_a.speed + t * (point_b.speed - point_a.speed)
            )
        end

        accumulated_length = accumulated_length + segment_length
    end
